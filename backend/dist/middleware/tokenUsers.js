    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                ...
                if (!token) {
                    return [2 /*return*/, res.status(401).json({ status: 'Unauthorized', message: 'Token não fornecido' })];
                }
                _b.label = 1;
            case 1:
                _b.trys.push([1, 3, , 4]);
                decoded = jwt.verify(token, process.env.secretToken);
                return [4 /*yield*/, SchemmaDatabaseUser_1.default.findOne({ _id: decoded.userId }).exec()];
            case 2:
                user = _b.sent();
                if (!user) {
                    return [2 /*return*/, res.status(401).json({ status: 'Unauthorized', message: 'Usuário não encontrado' })];
                }
            ...
            case 3:
                error_1 = _b.sent();
                console.error('Erro ao verificar token:', error_1);
                return [2 /*return*/, res.status(403).json({ status: 'Forbidden', message: 'Token inválido' })];
